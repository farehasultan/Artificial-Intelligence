...
'''
Consider a cat and a mouse in a room with four squares, as illustrated below. If the cat and mouse occupy the
same square, the mouse receives negative reward. If the cat and mouse occupy different squares, the mouse
receives positive reward. In this assignment, we will use temporal difference Q-learning to learn the optimal policy
for the mouse to achieve as much reward as possible.
...
| Square Y |  Square Z |
------------------------
The mouse agent can take the following actions: do not move or move to a horizontally or vertically adjacent
square.
Assume that this is a fully observable environment. That is, the mouse knows for all times its position and the cat’s
position.
Assume that this is a discrete time environment. At each time, the mouse agent may take one action. Furthermore,
at each time, with some probability, the cat may move to a horizontally or vertically adjacent square.
...
#td = td_qlearning("Inputs/Examples/Example0/trial.csv")
